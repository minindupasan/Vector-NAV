...
Minimal control: direct proportional PWM from cmd_vel.
No PID, no ramping, no dead zone compensation.
"""
...
class MotorDriverNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f'Motor driver started (MINIMAL) — sep={self.wheel_sep:.3f}m, '
            f'rad={self.wheel_rad:.3f}m, ticks/rev={ticks}, '
            f'max_speed={self.max_speed}m/s, rate={self.control_rate}Hz')
    ...
    def _control_loop(self):
        ...
        v_left = self.cmd_linear - (self.cmd_angular * self.wheel_sep / 2.0)
        v_right = self.cmd_linear + (self.cmd_angular * self.wheel_sep / 2.0)

        # --- Direct proportional PWM ---
        pwm_left = max(-1.0, min(1.0, v_left / self.max_speed))
        pwm_right = max(-1.0, min(1.0, v_right / self.max_speed))

        # Drive motors (same PWM for both motors on each side)
        ...
        # Update motor direction tracking for encoder reads
        if abs(pwm_left) > 0.01:
            self._motor_dir[0] = 1 if pwm_left > 0 else -1
            self._motor_dir[1] = 1 if pwm_left > 0 else -1
        if abs(pwm_right) > 0.01:
            self._motor_dir[2] = 1 if pwm_right > 0 else -1
            self._motor_dir[3] = 1 if pwm_right > 0 else -1
        ...
        js.velocity = [
            v_lf_meas / self.wheel_rad,
            v_lr_meas / self.wheel_rad,
            v_rf_meas / self.wheel_rad,
            v_rr_meas / self.wheel_rad,
        ]
